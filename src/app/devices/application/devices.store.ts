...
import {
  ...
  DisplayMode,
} from '../infrastructure/devices-api-endpoint';
...
export class DevicesStore {
  ...
  fetchDeviceById(id: string): Observable<Device> {
    return this.devicesApi.getDeviceById(id).pipe(tap((updated) => this._replaceDevice(updated)));
  }
  ...
  updateDisplayMode(deviceId: string, displayMode: DisplayMode): Observable<Device> {
    return this.devicesApi
      .updateDisplayMode(deviceId, displayMode)
      .pipe(tap((updated) => this._replaceDevice(updated)));
  }

  updateWithdrawnStock(deviceId: string, amount: number): Observable<Device> {
    return this.devicesApi
      .updateWithdrawnStock(deviceId, amount)
      .pipe(tap((updated) => this._replaceDevice(updated)));
  }
